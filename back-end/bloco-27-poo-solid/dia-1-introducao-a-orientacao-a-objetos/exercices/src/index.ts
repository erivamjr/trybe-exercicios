...
// Escreva uma classe cujos objetos representarão pessoas estudantes 
// matriculadas em uma disciplina. Cada objeto dessa classe deve conter os 
// seguintes dados: matrícula, nome, 4 notas de prova, 2 notas de trabalho.
...
class Students {
  ...
  constructor(matriculation: number, name: string, exam: number[], work: number[]) {
  ...
  sumMediaExam() {
    if (this.exam.length === 4) {
      console.log(this.exam.reduce((cu, ac) => cu = cu + ac));
...
const std1 = new Students(111, 'José Erivam', [2, 8, 10, 10], [7, 5])
std1.sumMediaExam();
